(function() {
        ...
        var newWorker = class Worker extends getCleanWorker(window.Worker) {
            constructor(twitchBlobUrl, options) {
                ...
                this.addEventListener('message', (e) => {
                    ...
                    } else if (e.data.key == 'UboPausePlayer') {
                        // New message: immediately pause the <video> element to prevent ad playback while we swap streams
                    ...
                    } else if (e.data.key == 'UboResumePlayer') {
                        const video = document.querySelector('video');
                        try {
    ...
    async function onFoundAd(streamInfo, textStr, reloadPlayer, realFetch, url, resolutionInfo) {
        ...
        // Pause the player immediately to avoid continuing to download/play ad segments
        postMessage({key:'UboPausePlayer'});
        // Ensure the player reloads to the backup/clean stream
        if (reloadPlayer) {
            postMessage({key:'UboReloadPlayer'});
        } else {
            // If caller didn't request a reload, force one to prevent ads from continuing
            postMessage({key:'UboReloadPlayer'});
        }
    ...
    async function processM3U8(url, textStr, realFetch) {
        ...
        if (streamInfo.BackupEncodings) {
            ...
            if (streamInfo.BackupEncodings && haveAdTags) {
                // Force a reload and pause to prevent continuing ad playback
            ...
            // When we detect ads on the currently used encoding, always force a swap + pause
            textStr = await onFoundAd(streamInfo, textStr, true, realFetch, url, currentResolution);
        } else {
            postMessage({key:'UboHideAdBanner'});
        }
                        ...
                        if (streamInfo == null || streamInfo.Encodings == null) {
                            ...
                            if (encodingsM3u8Response != null && encodingsM3u8Response.status === 200) {
                                ...
                                if (streamM3u8Response.status == 200) {
                                    var streamM3u8 = await streamM3u8Response.text();
                                    if (streamM3u8.includes(AD_SIGNIFIER) || SimulatedAdsDepth > 0) {
                                        await onFoundAd(streamInfo, streamM3u8, true, realFetch, resolutionInfo.Url, resolutionInfo);
                                    }
                                } else {
